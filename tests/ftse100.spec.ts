...
test.describe('FTSE 100', () => {
  ...
  test('latest top 10 constituents with the highest percentage change', async () => {
    ...
    await expect(FTSE100Page).toHaveURL(/\/indices\/ftse-100\/constituents\/table/, {
      ignoreCase: true,
      timeout: 20000,
    });

    //Verify the user is in FTSE 100 index page
    await expect(FTSE100Page.getByRole('heading', { name: 'FTSE 100' })).toBeVisible({
      timeout: 5000,
    });

    // Verify the table has been selected
    await expect(FTSE100Page.getByRole('listitem').filter({ hasText: /Table/i })).toHaveClass(
      /active/
    );
  ...
  test('latest top 10 constituents with the lowest percentage change', async () => {
    ...
    console.log('FTSE 100’s latest top 10 constituents with the lowest percentage change');
    console.table(rows);
  });
